      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
          {leaderboardUsers.map((user, index) => (
            <Card
              key={index}
              className={`overflow-hidden border-none shadow-lg ${user.bgClass} relative p-6 ${index === 1 ? "md:scale-110" : "md:scale-90"}`}
            >
              <span
                className={`absolute top-4 left-4 text-6xl font-bold opacity-70 md:text-7xl ${user.textClass}`}
              >
                {user.rank}
              </span>
